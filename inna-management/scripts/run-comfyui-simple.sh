...
# Simple approach: Reconstruct the data and call the tool via Open WebUI API
...
echo -e "${BLUE}🎨 Running ComfyUI for existing personas...${NC}"
...
# Create Python script to reconstruct data and manually run the ComfyUI workflow
cat > /tmp/manual_comfyui.py << 'EOF'
import json
import os
import sys
import requests
...
def infer_posture_gesture(behaviors, quote):
    # Simple inference based on behaviors and quote
...
def submit_to_comfy(comfy_url, workflow):
    payload = {
        "prompt": workflow,
        "client_id": f"inna-manual-{hash(str(workflow)) % 10000}"
    }
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python manual_comfyui.py <project_dir> <project> <batch_id>")
        sys.exit(1)
    ...
    summary_file = Path(project_dir) / "manual_comfyui_jobs.json"
...
echo -e "${YELLOW}🔄 Processing personas and submitting to ComfyUI...${NC}"
python3 /tmp/manual_comfyui.py "$PROJECT_DIR" "$PROJECT" "$BATCH_ID"
